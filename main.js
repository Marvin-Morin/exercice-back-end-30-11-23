...
const supprimerRecette = require('./modules/supprimerRecette'); // Ajout de l'importation de l méthode pour supprimer une recette
...
async function main() { // Je créer une fonction pour ajouter une nouvelle recette et lire le fichier txt
    try {
        ...
        await addRecette(recettes, 'Moules frites', 'Moules, frites'); // J'importe une recette graçe à ma méthode addRecette dans mon fichier addRecette
        console.log('Recette(s) ajouté(s) avec succès'); // Je vérifie si tout c'est bien déroulé
        await lireFichier('Recettes.txt') // puis je lis le fichier txt
        // await supprimerRecette('Recettes.txt', 'Poulet frites'); // Suppression d'une recette
